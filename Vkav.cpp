...
namespace {
	...
	static constexpr const char* helpStr =
	    ...
	    "programs execution.\n"
	    "\t-s, --sink-name=SINK          Uses SINK instead of the default "
	    "audio "
	    "sink.(Overrides sink specified in the config file.)\n"
	    "\t-d, --device=DEVICE_NUMBER    Uses Device number "
	    "DEVICE_NUMBER.(Overrides device number specified in the config "
	    "file.)\n"
	    "\t-c, --config-file=CONFIG_PATH Specifies the path to the "
	    "configuration "
	    "file to use.\n"
	    "\t-a, --amplitude=AMPLITUDE     Changes the amplitude of the fft "
	    "output.\n"
