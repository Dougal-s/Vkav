...
void main() {
	...
	vec3 view = vec3(0, yPos-2*floorY, cameraZ);
	camera = view;
	...
	localPos = model*(vec3(-1,1,1)*positions[gl_VertexIndex])+translate;
	gl_Position = projection*vec4(vec3(1,-1,1)*(localPos-view), 1.0);
